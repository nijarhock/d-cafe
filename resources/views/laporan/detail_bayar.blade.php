    ...
    <table class="table">
        <thead>
            <tr>
                ...
                <th>PPN %</th>
                <th>Grand Total</th>
                @foreach($jenis_byr as $bayar)
                @php ${"bayar" . $bayar->id} = 0; @endphp
                <th>{{ $bayar->nama }}</th>
                @endforeach
            </tr>
        </thead>
        <tbody>
            ...
            @foreach($pesanan as $data)
                ...
                <tr>
                    ...
                    ${"bayar" . $bayar->id} += $bayar->sumBayar($bayar->id, $data->id);
                    @endphp
                    <td>{{ number_format($bayar->sumBayar($bayar->id, $data->id)) }}</td>
                    @endforeach
                </tr>
            @endforeach 
            <tr>
                ...
                @foreach($jenis_byr as $bayar)
                <th>{{ number_format(${"bayar" . $bayar->id}) }}</th>
                @endforeach
            </tr>
        </tbody>
    </table>
